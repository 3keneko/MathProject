...
\author{Avec une assistance minimale de Evrard Maurice,\\ Mathieu Louca,\\ Pluvinage Victor\\ et de Ralet Vincent.}

\begin{document}
\begin{center}
  {\Huge {\bf Un projet de LEJEUNE Productions}}
\end{center}
\maketitle
\newpage
...
\subsection{Origines.}
\begin{enumerate}
  \item 1949 dans une publication de Warren Weaver\footnote{Warren Weaver, mathématicien américain, considéré comme la 1ère personne à proposer l’utilisation d’ordinateur électronique pour la traduction grâce à des techniques statistiques} par rapport à la traduction automatique des langues.
  \item 1950 dans un article d’Alan Turing\footnote{Alan Turing: Histoire dit de lui qu’il a dechiffrer un code secret des machines Enigma utilisé par les allemands à l’aide Bletchley Park(un château rempli de mathématicien) qui aurait permis de raccourcir la seconde guerre mondiale de 2 ans} qui définit si une machine est consciente ou non.
\end{enumerate}
\subsection{Officialisation de l’IA comme véritable domaine scientifique.}
\begin{enumerate}
  \item 1956 lors d’une conférence aux USA qui permettra à celle-ci d’atteindre des université prestigieuse comme stanford, etc
\end{enumerate}
\subsection{Suite de l'IA.}
\begin{enumerate}
  \item 1974 arrive le “Al Winter”. Réduction du budget des académies car elles ne font pas aboutir leur projet sur quelque chose de concret
  \item Année 80, Gouvernements financent de nouveau les académies car IA atteint une valeur d’un milliard de dollars, a ce moment, un système expert était un ordinateur capable de se comporter comme un humain (dans un domaine précis)
  \item la loi de Moore (évolution de la puissance de calcul des ordinateurs ainsi que de leurs complexités), entre 1990 et 2000, On trouve le data mining (composante des technologies)
  \item 1997: Deep blue (superintelligence spécialisé dans le jeu d’échecs)
\end{enumerate}
