...
\end{lstlisting}
\end{enumerate}
\subsubsection{Implémentons un algorithme Glouton!}
Dans cette partie, nous allons vous montrer une implémentation d'un algorithme glouton.\\

Pour illustrer, nous allons créer un programme qui, quand on lui donne un montant, va nous rendre une somme convertie en billets de 500 euros, de 200 euros, de 100 euros et ainsi de suite jusqu'au pièces de 1 et 2 euros. Pour plus d'information sur les algorithmes gloutons, nous vous invitons à lire la page wikipédia à ce sujet. \url{https://fr.wikipedia.org/wiki/Algorithme_glouton}\\

Passons directement aux explications.\\

\lstinputlisting[language=Python, linerange={1-1}]{PythonIsh/money.py}
Tout d'abord, nous déclarons une liste contenant la valeur de chaque billet. Remarquez que nous nommons cette liste en toute majuscules, nous ne faisons que respecter une convention voulant que les constantes soient nommées ainsi.

\lstinputlisting[language=Python, linerange={10-11}]{PythonIsh/money.py}
Ensuite, nous demandons un montant en euros à l'utilisateur, et nous initialisons res, un dictionnaire vide. Celui-ci aura pour objectif de relier les valeurs de chaque billet, au nombre de billets qui devra être retourné.

\lstinputlisting[language=Python, linerange={13-15}]{PythonIsh/money.py}
Ici, nous itérons à travers chaque élément de CASH, et utilisons la fonction divmod, prédéfinie à python, qui pourrait se définir ainsi:

\begin{lstlisting}[language=Python]
divmod = lambda a, b: a // b, a % b
\end{lstlisting}

Par la suite, nous définissons la fonction money\_print, celle-ci nous à rendre la sortie de programme plus agréable à lire (en effet, avoir une belle ribambelle de print est plus agréable à l'œil qu'un dictionnaire brut.)
\lstinputlisting[language=Python, linerange={4-7}]{PythonIsh/money.py}
Le lecteur pourra remarquer l'utilisation de ternaires, celles-ci servent avant tout à alléger la syntaxe des conditions.

\lstinputlisting[language=Python, linerange={17-18}]{PythonIsh/money.py}
Enfin, nous itérons à travers chaque élément de res et montrons le résultat proprement affiché.
\begin{lstlisting}[language=Python]
Veuillez insérer un montant: 547
1 billet(s) de 500 euro(s).
2 billet(s) de 20 euro(s).
1 billet(s) de 5 euro(s).
1 pièce(s) de 2 euro(s).
\end{lstlisting}
