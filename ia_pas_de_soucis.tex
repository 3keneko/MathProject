...
Tous les programmes créés dans ce projet sont disponibles à l'adresse suivante: \begin{center}
...
3
\end{lstlisting}
Il faut toutefois prendre en compte le fait que les listes simplement chaînées ne sont pas des structures de données très utilisées dans tout les langages de programmation. Par exemple, en Python, il faut importer la libraire llist afin de pouvoir les utiliser. \\
...
0.8333333
\end{lstlisting}
Comme on peut le voir, la logique reste la même pour tous les opérateurs, aussi, cette syntaxe nous permet également de mettre à ces opérateurs autant d'arguments qu'on le souhaite, (l'exemple est donné ci-dessus avec l'opérateur de multiplication.). \\
...
Également, j'utilise \# , ce symbole nous permet d'expliciter que nous utilisons une fonction, et pas une variable.\\
...
[[1 2 3]
 [4 5 6]]
\end{lstlisting}
Attention, ne toujours pas oublier de mettre tous les chiffres entre [ ] ou entre ( ) pour créer le tableau. \\
...
\connect{329-363}
Voici enfin la fin du programme Lisp, avec la partie interface utilisateur, le programme permet de jouer contre un autre joueur, ou encore de jouer contre l'IA. \\
